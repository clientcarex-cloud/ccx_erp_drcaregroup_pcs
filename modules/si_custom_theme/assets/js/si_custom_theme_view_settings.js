...
function edit_custom_theme_name(theme_id)
{
	requestGet(admin_url + 'si_custom_theme/get_theme_name_data_ajax/' + theme_id).done(function(response) {
    	$('#div_si_ct_edit_custom_theme_name').html(response);
		setTimeout(function() {
			$('#si_ct_edit_custom_theme_name_modal').modal('show');
			init_color_pickers();
			$('#si_ct_edit_custom_theme_name_modal .colorpicker-input').colorpicker().on('changeColor', function(e) {
				var _this = $(this).find('input');
				var id = _this.data('id');
				$(id).css('background-color',_this.val());
			});
		}, 150);
        
    }).fail(function(data) {
        $('#si_task_note_model').modal('hide');
