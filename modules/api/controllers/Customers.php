...
class Customers extends REST_Controller {
    ...
    public function birthday_get($key = '') {
        // If the id parameter doesn't exist return all the
        $data = $this->Api_model->search('customer_birthday', $key);
        // Check if the data store contains
        if ($data) {
            //$data = $this->Api_model->get_api_custom_data($data, "customers");
            // Set the response and exit
    ...
    public function staff_birthday_get($key = '') {
        // If the id parameter doesn't exist return all the
        $data = $this->Api_model->search('staff_birthday', $key);
        // Check if the data store contains
        if ($data) {
            // Set the response and exit
            $this->response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
