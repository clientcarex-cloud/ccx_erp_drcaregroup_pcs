...
class Leads extends REST_Controller {
    ...
    public function data_post() {
		...
		$junk = 0;
		if (is_dir(FCPATH . 'modules/lead_rollercoaster')) {
			 $this->load->model('lead_rollercoaster/Lead_rollercoaster_model');
			$roller_settings = $this->Lead_rollercoaster_model->get_settings();
			if (!empty($roller_settings->auto_junk_leads)) {
				
				$junk_rules = json_decode($roller_settings->junk_lead_rules, true);

				if (is_array($junk_rules)) {
					foreach ($junk_rules as $rule) {
						if (isset($rule['country_code'], $rule['expected_length'])) {
							$expected_length = (int) $rule['expected_length'];
							if(strlen($this->input->post('phonenumber')) != $expected_length){
								$junk = 1;
							}
						}
					}
			...
			if (!empty($roller_settings->avoid_empty_leads)) {
				...
				if (!empty($roller_settings->avoid_empty_leads)) {
					if ($name === '' || $phonenumber === '') {
						$message = array(
							'status' => FALSE,
							'message' => 'Lead not added. Name and Mobile Number are mandatory.'
						);
