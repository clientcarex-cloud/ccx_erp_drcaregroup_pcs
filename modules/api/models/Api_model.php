...
class Api_model extends App_Model
{
    ...
    public function get_table($name, $id)
    {
        ...
        switch ($name) {
            ...
            case 'testimonial':
                if (is_numeric($id)) {
                    return $this->get_testimonial($id);
                } else {
                    $result = [];
                    $testimonials = $this->db->get(db_prefix() . 'flextestimonial')->result_array();
                    foreach ($testimonials as $testimonial) {
                        $result[] = (array)$this->get_testimonial($testimonial['id']);
                    }
                    return $result;
                }
                break;
			...
			case 'flexform':
				if (is_numeric($id)) {
					return $this->get_flexform($id);
				} else {
					$result = [];
					$flexforms = $this->db->get(db_prefix() . 'flexforms')->result_array();
					foreach ($flexforms as $flexform) {
						$result[] = (array)$this->get_flexform($flexform['id']);
					}
					return $result;
				}
				break;	
            ...
            case 'knowledge_base':
                ...
                if (is_numeric($id)) {
                    ...
                    $result = [];
                    $knowledge_bases = $this->db->get(db_prefix() . 'knowledge_base')->result_array();
                    foreach ($knowledge_bases as $knowledge_base) {
                        $result[] = (array)$this->Knowledge_base_model->get($knowledge_base['articleid']);
                    }
                    return $result;
                }
                break;
            ...
            case 'todos':
                if (is_numeric($id)) {
                    return $this->get_todos($id);
                } else {
                    $result = [];
                    $todos = $this->db->get(db_prefix() . 'todos')->result_array();
                    foreach ($todos as $todo) {
						
                        $result[] = (array)$this->get_todos($todo['todoid']);
                    }
                    return $result;
                }
                break;
            ...
            case 'reminders':
                if (is_numeric($id)) {
                    return $this->get_reminders($id);
                } else {
                    $result = [];
                    $reminders = $this->db->get(db_prefix() . 'reminders')->result_array();
                    foreach ($reminders as $reminder) {
						
                        $result[] = (array)$this->get_reminders($reminder['id']);
                    }
                    return $result;
                }
                break;
    ...
    public function _search_todos($q, $limit = 0, $api = false)
    {
        $result = [
            'result'         => [],
            'type'           => 'todos',
            'search_heading' => _l('todos'),
        ];
        if (has_permission('todos', '', 'view') || true == $api) {
            // todos
            $fields = get_custom_fields('todos');
            $this->db->select(db_prefix().'todos.*');
            $this->db->from(db_prefix().'todos');
            $this->db->like('description', $q);
            $this->db->or_like(db_prefix().'todos.todoid', $q);
            foreach ($fields as $key => $value) {
                $this->db->join(db_prefix().'customfieldsvalues as ctable_'.$key.'', db_prefix().'todos.todoid = ctable_'.$key.'.relid and ctable_'.$key.'.fieldto="todos" AND ctable_'.$key.'.fieldid='.$value['todoid'], 'LEFT');
                $this->db->or_like('ctable_'.$key.'.value', $q);
            }
	...
	public function _search_testimonial($q, $limit = 0, $api = false)
	{
		$result = [
			'result'         => [],
			'type'           => 'testimonial',
			'search_heading' => _l('testimonial'),
		];

		if (has_permission('flextestimonial', '', 'view') || true == $api) {
			$fields = get_custom_fields('flextestimonial');
			$this->db->select(db_prefix() . 'flextestimonial.*');
			$this->db->from(db_prefix() . 'flextestimonial');
			$this->db->like('title', $q);
			$this->db->or_like('slug', $q);
			$this->db->or_like('description', $q);
			$this->db->or_like('welcome_title', $q);
			$this->db->or_like('thankyou_title', $q);
			$this->db->or_like('thankyou_message', $q);
			$this->db->or_like('notification_emails', $q);

			foreach ($fields as $key => $field) {
				$this->db->join(
					db_prefix() . 'customfieldsvalues as cft_' . $key,
					db_prefix() . 'flextestimonial.id = cft_' . $key . '.relid AND cft_' . $key . '.fieldto="flextestimonial" AND cft_' . $key . '.fieldid=' . $field['id'],
					'LEFT'
				);
				$this->db->or_like('cft_' . $key . '.value', $q);
			}
			...
			$testimonials = $this->db->get()->result_array();

			foreach ($testimonials as &$testimonial) {
				$testimonial_id = $testimonial['id'];
				$testimonial['responses'] = $this->db->select('*')
					->from(db_prefix() . 'flextestimonialresponses')
	...
	public function _search_knowledge_base($q, $limit = 0, $api = false)
	{
		$result = [
			'result'         => [],
			'type'           => 'knowledge_base',
			'search_heading' => _l('knowledge_base'),
		];

		if (has_permission('knowledge_base', '', 'view') || $api === true) {
			$fields = get_custom_fields('knowledge_base');
			$this->db->select(db_prefix() . 'knowledge_base.*');
			$this->db->from(db_prefix() . 'knowledge_base');

			$this->db->like('subject', $q);
			$this->db->or_like('description', $q);
			$this->db->or_like('slug', $q);
			$this->db->or_like(db_prefix() . 'knowledge_base.articleid', $q);
			...
			foreach ($fields as $key => $field) {
				...
				$this->db->join(
					...
					db_prefix() . 'knowledge_base.articleid = ' . $alias . '.relid AND ' . $alias . '.fieldto = "knowledge_base" AND ' . $alias . '.fieldid = ' . $field['id'],
					'LEFT'
				);
				$this->db->or_like($alias . '.value', $q);
			}
    ...
    public function get_relation_data_api($type, $search = '')
    {
        ...
        if ('customer' == $type || 'customers' == $type) {
			...
			if ($q) {
				...
				foreach ($fields as $key => $value) {
					$this->db->join(
						...
						db_prefix().'clients.userid = ctable_'.$key.'.relid 
						 AND ctable_'.$key.'.fieldto = "customers" 
						 AND ctable_'.$key.'.fieldid = '.$value['id'],
						'LEFT'
					);
			...
			$this->load->model('clients_model');
			$clients = $this->clients_model->get('', $where_clients);

			$data = [];

			foreach ($clients as $client) {
				// Get call logs for this patient
				$call_logs = $this->db->where('patientid', $client['userid'])
									  ->order_by('created_date', 'DESC')
									  ->get('tblpatient_call_logs')
									  ->result_array();

				// Append call_logs to each client
				$client['call_logs'] = $call_logs;

				$data[] = $client;
			}
		}elseif ('customer_birthday' == $type) {
			$today = date('m-d');
			...
			$this->db->join(db_prefix() . 'clients_new_fields as nf', 'c.userid = nf.userid', 'LEFT');
			$this->db->where('c.active', 1);
			$this->db->where('DATE_FORMAT(nf.dob, "%m-%d") =', $today);
            ...
            $search = $this->_search_todos($q, 0, true);
            $data   = $search['result'];
        } elseif ('testimonial' == $type) {
            $search = $this->_search_testimonial($q, 0, true);
            $data   = $search['result'];
        } elseif ('knowledge_base' == $type) {
            $search = $this->_search_knowledge_base($q, 0, true);
            $data   = $search['result'];
   ...
   //  update subscriptions data
     ...
     * @return array
     */
    public function get_todos_items($finished, $page = '')
    {
        $this->db->select();
        $this->db->from(db_prefix().'todos');
	...
	public function delete_todos_item($id)
    {
        $this->db->where('todoid', $id);
        //$this->db->where('staffid', get_staff_user_id());
	...
	public function add_knowledge_article($data)
	{
		// Handle group logic
		$group = $this->db->get_where('tblknowledge_base_groups', ['name' => $data['articlegroup']])->row();

		if (!$group) {
			// Create new group
			$this->db->insert('tblknowledge_base_groups', [
				'name'        => $data['articlegroup'],
				'group_slug'  => slug_it($data['articlegroup']),
				'description' => '',
				'active'      => 1
			]);
		...
		$this->db->insert('tblknowledge_base', $article);
		return $this->db->insert_id();
	}
	public function update_knowledge_base($data, $id)
	{
		// Extract and remove custom fields from data if present
		$custom_fields = isset($data['custom_fields']) ? $data['custom_fields'] : null;
		unset($data['custom_fields']);

		// Update slug from subject if needed
		...
		if ($this->db->affected_rows() > 0) {
			...
			if (!empty($custom_fields)) {
				handle_custom_fields_post('knowledge_base', $custom_fields, $id);
			}
	...
	public function update_todo($data, $id)
	{
		...
		if ($this->db->affected_rows() > 0) {
			...
			if (!empty($custom_fields)) {
				handle_custom_fields_post('todos', $custom_fields, $id);
			}
	...
	public function get_testimonial($active = '1', $page = '')
	{
		$this->db->select('*');
		$this->db->from(db_prefix() . 'flextestimonial');
		...
		foreach ($testimonials as &$testimonial) {
			// Format dates
			$testimonial['created_at'] = _dt($testimonial['created_at']);
			$testimonial['updated_at'] = _dt($testimonial['updated_at']);
	...
	public function add_reminder($data)
	{
		...
		$insert_data = [
			...
			'isnotified'       => isset($data['isnotified']) ? (int)$data['isnotified'] : 0,
			'rel_id'           => isset($data['rel_id']) ? (int)$data['rel_id'] : 0,
			'staff'            => isset($data['staff']) ? (int)$data['staff'] : 0,
			'rel_type'         => isset($data['rel_type']) ? $data['rel_type'] : '',
			'notify_by_email'  => isset($data['notify_by_email']) ? (int)$data['notify_by_email'] : 1,
			'creator'          => isset($data['creator']) ? (int)$data['creator'] : 0,
		];

		$this->db->insert(db_prefix() . 'reminders', $insert_data);
		return $this->db->insert_id();
	}
	...
	public function update_reminder($data, $id)
	{
		...
		// Trim keys in $data to remove trailing/leading whitespace or newlines
		$clean_data = [];
		...
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'reminders', $clean_data);
	...
	public function get_flexform($id) {
		$flexform = $this->db->get_where(db_prefix() . 'flexforms', ['id' => $id])->row();

		if (!$flexform) {
			return null;
		}

		// Add any custom data processing here if needed, e.g.
		// $flexform->some_field = custom_processing($flexform->some_field);
