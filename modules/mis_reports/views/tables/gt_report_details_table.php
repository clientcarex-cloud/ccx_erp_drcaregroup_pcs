...
// Base tables type
$base_type = 'payment'; // default
if (in_array($cell_type, ['np_visits', 'ren_visits', 'ref_visits'])) {
    $base_type = 'appointment';
} elseif ($cell_type == 'refund') {
    $base_type = 'refund';
}
...
if ($base_type == 'appointment') {
    ...
    if ($cell_type == 'np_visits') {
        ...
        $where_clause .= " AND NOT EXISTS (SELECT 1 FROM tblgoal_lead_sources gls WHERE gls.source_id = cn.patient_source_id AND gls.category = 'referral')";
    } elseif ($cell_type == 'ren_visits') {
        $where_clause .= " AND atype.appointment_type_name <> 'First Appointment'";
    } elseif ($cell_type == 'ref_visits') {
        $where_clause .= " AND atype.appointment_type_name = 'First Appointment'";
        $where_clause .= " AND EXISTS (SELECT 1 FROM tblgoal_lead_sources gls WHERE gls.source_id = cn.patient_source_id AND gls.category = 'referral')";
    }
    ...
    $select_sql = "
        SELECT 
            ...
            (SELECT MIN(appointment_date) FROM tblappointment WHERE userid = a.userid) as first_visit_date,
        ...
        LEFT JOIN tblclients_new_fields cn ON cn.userid = a.userid
        LEFT JOIN tblleads_sources sources ON sources.id = cn.patient_source_id
        LEFT JOIN tblitems items ON items.id = a.treatment_id
        LEFT JOIN tblstaff staff ON staff.staffid = a.doctor_id
        WHERE $where_clause
    ";
    ...
    $select_sql = "
        SELECT 
            c.company as patient_name, cn.mr_no, sources.name as source_name, items.description as treatment_name,
            atype.appointment_type_name as appt_type_name, c.datecreated as created_date,
            (SELECT MIN(appointment_date) FROM tblappointment WHERE userid = c.userid) as first_visit_date,
            a.appointment_date as appointment_date, a.visited_date as visited_date, a.consulted_date as consulted_date,
            CONCAT(staff.firstname, ' ', staff.lastname) as doctor_name, cn.registration_start_date as registration_date,
        ...
        LEFT JOIN tblitemable item ON item.rel_id = inv.id AND item.rel_type = 'invoice' 
        LEFT JOIN tblclients_new_fields cn ON cn.userid = c.userid
        LEFT JOIN tblleads_sources sources ON sources.id = cn.patient_source_id
        LEFT JOIN tblpayment_modes pm ON pm.id = pr.paymentmode
        LEFT JOIN tblappointment a ON a.appointment_id = (
             SELECT a_sub.appointment_id FROM tblappointment a_sub 
             WHERE a_sub.userid = c.userid AND DATE(a_sub.appointment_date) <= pr.date
             ORDER BY a_sub.appointment_date DESC LIMIT 1
        )
        LEFT JOIN tblappointment_type atype ON atype.appointment_type_id = a.appointment_type_id
        LEFT JOIN tblitems items ON items.id = a.treatment_id
        LEFT JOIN tblstaff staff ON staff.staffid = a.doctor_id
        WHERE $where_clause
    ";
    
    // Grouping depends on if we just count unique patients (registrations) or sum payments
    ...
    $select_sql = "
        ...
        JOIN tblcreditnotes cn_inv ON cn_inv.id = cr.credit_note_id
        JOIN tblcustomer_groups map ON map.customer_id = cn_inv.clientid
        JOIN tblclients c ON c.userid = cn_inv.clientid
        LEFT JOIN tblclients_new_fields cn ON cn.userid = c.userid
        LEFT JOIN tblleads_sources sources ON sources.id = cn.patient_source_id
        LEFT JOIN tblpayment_modes pm ON pm.id = cr.payment_mode
        LEFT JOIN tblappointment a ON a.appointment_id = (
             SELECT a_sub.appointment_id FROM tblappointment a_sub 
...
if ($search) {
    // Basic search filtering wrapping the whole query
    ...
    $final_sql = "
        ...
        WHERE filtered.patient_name LIKE '%$search_esc%'
           ...
           OR filtered.source_name LIKE '%$search_esc%'
    ";
} else {
...
foreach ($results as $row) {
    $data[] = [
        ...
        e($row['appt_type_name']),
        _d($row['created_date']),
        _d($row['first_visit_date']),
        _d($row['appointment_date']),
        _d($row['visited_date']),
        _d($row['consulted_date']),
        e($row['doctor_name']),
        _d($row['registration_date']),
        $row['package_amount'] ? app_format_money_custom($row['package_amount'], 1) : '',
        $row['paid_amount'] ? app_format_money_custom($row['paid_amount'], 1) : '',
        _d($row['payment_date']),
        e($row['payment_type']),
    ];
}
