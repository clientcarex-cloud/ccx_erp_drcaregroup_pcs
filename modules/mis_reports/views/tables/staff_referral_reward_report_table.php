...
// Filter parameters from GET (query string)
...
$CI->db->select("
    ...
    l.junk as lead_junk,
    l.lost as lead_lost,
    s.staffid as referrer_id,
    s.firstname as referrer_firstname,
    s.lastname as referrer_lastname,
...
// Join referrer staff
$CI->db->join(db_prefix() . 'staff s', 's.staffid = l.refer_id', 'left');
...
// Join to check if lead converted to client
$CI->db->join(db_prefix() . 'clients c', 'c.leadid = l.id', 'left');

// Join clients_new_fields for MR No
$CI->db->join(db_prefix() . 'clients_new_fields cnf', 'cnf.userid = c.userid', 'left');

// Filter: only staff referrals
$CI->db->where('l.refer_type', 'staff');
...
if (!empty($search_value)) {
    ...
    $CI->db->or_like('s.firstname', $search_value);
    $CI->db->or_like('s.lastname', $search_value);
...
// Clone for count before applying limit
...
foreach ($results as $row) {
    $referrer_name = trim(($row['referrer_firstname'] ?? '') . ' ' . ($row['referrer_lastname'] ?? ''));
