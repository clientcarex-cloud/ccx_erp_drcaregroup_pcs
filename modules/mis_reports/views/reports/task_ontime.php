...
<div class="row align-items-end">
    ...
    <div class="col-md-3">
	 ...
	 <div class="col-md-2">
        ...
        <input class="form-control" type="date" id="consulted_date" name="consulted_date" value="<?= html_escape(set_value('consulted_date') ?: date('2011-01-01')) ?>">
    </div>
...
$columns = [
    ...
    _l('OnTime'),
    _l('Grand Total'),
    _l('Percentage Completed'),
];
echo render_datatable($columns, 'timeliness_report');
?>
...
function initAppointmentsTable() {
    ...
    if ($.fn.DataTable.isDataTable('.table-timeliness_report')) {
        $('.table-timeliness_report').DataTable().destroy();
    }
    
    // Initialize new table
    initDataTable('.table-timeliness_report', url, [1], [1]);
}
</script>
    ...
    $('#searchAppointmentsBtn').on('click', function () {
        ...
        if ($.fn.DataTable.isDataTable('.table-appointments')) {
            $('.table-appointments').DataTable().ajax.url(
                '<?= admin_url("mis_reports/reports/$type/1/") ?>' + from + '/' + to + '/' + appointmentType + '/' + branchId + '/' + roleId
            ).load();
        }
    });
});
