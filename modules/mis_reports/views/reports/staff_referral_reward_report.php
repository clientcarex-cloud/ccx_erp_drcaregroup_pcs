                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <form method="post" id="referral-filter-form"
                            action="<?= admin_url('mis_reports/staff_referral_reward_report'); ?>">
...
<script>
    $(function () {
        ...
        var baseUrl = '<?= admin_url("mis_reports/staff_referral_reward_report/") ?>';
        initDataTable('.table-staff-referral-reward', baseUrl + from + '/' + to, [0], [0]);

        $('#searchBtn').on('click', function () {
            var from = $('#consulted_date').val();
            var to   = $('#consulted_to_date').val();
            if ($.fn.DataTable.isDataTable('.table-staff-referral-reward')) {
                $('.table-staff-referral-reward').DataTable().ajax.url(baseUrl + from + '/' + to).load();
            } else {
                initDataTable('.table-staff-referral-reward', baseUrl + from + '/' + to, [0], [0]);
            }
        });
