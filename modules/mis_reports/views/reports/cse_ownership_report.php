...
<form method="post" action="<?= admin_url('mis_reports/reports/cse_ownership_report'); ?>"> 
...
<?php echo render_datatable([
    ...
    _l('call_type'),
    _l('call_owner'),
    _l('ownership_start_date'),
    _l('ownership_end_date'),
    _l('comments'),
], 'appointments'); ?>
    ...
    $('#searchAppointmentsBtn').on('click', function () {
        ...
        if ($.fn.DataTable.isDataTable('.table-appointments')) {
            $('.table-appointments').DataTable().ajax.url(
                '<?= admin_url("mis_reports/reports/$type/1/") ?>' + from + '/' + to + '/' + appointmentType + '/' + branchId + '/' + doctorId
            ).load();
        }
    });
});
