...
<form method="post" action="<?= admin_url('mis_reports/reports/branch_visit_report'); ?>">
...
$(function(){
    function loadAppointmentsTable() {
        ...
        if ($.fn.DataTable.isDataTable('.table-appointments')) {
            $('.table-appointments').DataTable().ajax.url('<?= admin_url("mis_reports/reports/$type/1/") ?>' + consulted_date + '/' + consulted_to_date).load();
        } else {
            initDataTable('.table-appointments', '<?= admin_url("mis_reports/reports/$type/1/") ?>' + consulted_date + '/' + consulted_to_date, [1], [1]);
        }
    }
