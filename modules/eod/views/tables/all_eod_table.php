...
foreach ($results as $row) {
    ...
    <div class="dropdown" style="display:inline-block;">
        <button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <i class="fa fa-check-circle"></i> ' . _l('change_status') . '
        </button>
        <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in">
            <a class="dropdown-item text-success" href="javascript:void(0);" onclick="updateEodStatus(' . $row['id'] . ', \'Approved\')">
                &emsp; <i class="fa fa-thumbs-up"></i> ' . _l('approved') . '
            </a><br>
            <a class="dropdown-item text-danger" href="javascript:void(0);" onclick="updateEodStatus(' . $row['id'] . ', \'Disapproved\')">
                &emsp; <i class="fa fa-thumbs-down"></i> ' . _l('disapproved') . '
            </a>
        </div>
    </div>';
