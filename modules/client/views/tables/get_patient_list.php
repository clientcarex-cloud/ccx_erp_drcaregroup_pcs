...
$normalizeBranchList = static function ($value) {
    ...
    foreach ($list as $item) {
        ...
        if ($decoded === '' || strtolower($decoded) === 'null') continue;
...
$restrictToAllowedBranches = static function ($ids) use ($allowedBranchIds) {
    if (empty($allowedBranchIds)) return $ids;
    if (empty($ids)) return [];
    $filtered = [];
    foreach ($ids as $id) {
        $intId = (int) $id;
...
$applyBranchFilter = static function ($query) use ($branchFilterIds) {
    if (empty($branchFilterIds)) return;
    $cleanIds = array_filter(array_map('intval', $branchFilterIds), function ($v) { return $v > 0; });
...
// Reusable summary filter closure — eliminates triple code duplication
...
// ── Pre-fetch leads_status colour map ONCE (moved before loops) ──
$statusColorMap = [];
...
// ══════════════════════════════════════════════════════════════
// MAIN DATA QUERY
// Branch names are fetched separately in a single batch query
...
// ── Batch-fetch branch names for the result set (replaces per-row subquery) ──
...
// ── Batch-fetch related data (appointments, call logs, journey statuses) ──
$treatmentMap = $doctorMap = $callLogMap = $leadStatuses = [];

if (!empty($userIds)) {
    $userIdsStr = implode(',', $userIds);

    // Latest appointment per patient (single optimised query)
    $CI->db->select('
        a.userid,
        a.enquiry_doctor_id,
    ...
    // Removed redundant where_in — the INNER JOIN on derived table already constrains rows
    ...
    // Latest journey status per patient (using derived-table MAX join)
