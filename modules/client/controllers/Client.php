...
class Client extends AdminController
{
	...
	public function index($id = '', $tab = '')
	{
		...
		if ($id) {
			$data['clientid'] = $id;
			$data['client_modal'] = $this->_get_patient_modal_html($id);
		}
	...
	public function appointments($id = NULL, $consulted_date = NULL, $consulted_to_date = NULL, $enquiry_doctor_id = NULL, $visit_status = NULL, $branch_id = NULL, $selected_branch_id = NULL, $appointment_type_id = NULL)
	{
		...
		if ($id) {
			$data['clientid'] = $id;
			$data['client_modal'] = $this->_get_patient_modal_html($id, $callback_url ?? null, $statuses ?? []);
		}
	...
	public function get_journey_log($userid)
	{
		...
		if (count($logs) >= 2) {
			...
			$tatString = '';
			if ($days > 0)
				$tatString .= $days . ' day' . ($days > 1 ? 's ' : ' ');
			if ($hours > 0)
				$tatString .= $hours . ' hour' . ($hours > 1 ? 's ' : ' ');
			if ($minutes > 0)
				$tatString .= $minutes . ' minute' . ($minutes > 1 ? 's ' : ' ');
			if ($seconds > 0 || $tatString === '')
				$tatString .= $seconds . ' second' . ($seconds != 1 ? 's' : '');
	...
	private function _get_patient_modal_html($id, $callback_url = NULL, $statuses = [])
	{
