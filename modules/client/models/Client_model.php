...
class Client_model extends App_Model
{
	...
	public function save_client()
	{
		...
		//Get Visit id - use direct SQL to avoid CI query builder state reset
		$countRow = $this->db->query(
			'SELECT COUNT(*) AS cnt FROM ' . db_prefix() . 'appointment WHERE DATE(appointment_date) = ?',
			[date('Y-m-d')]
		)->row();
		...
		$get_branch_code = $this->db->query(
			'SELECT options FROM ' . db_prefix() . 'master_settings WHERE title = ? AND branch_id = ? LIMIT 1',
			['branch_code', $branch_id]
		)->row();
		$branch_code = $get_branch_code ? $get_branch_code->options : '';

		$get_branch_short_code = $this->db->query(
			'SELECT options FROM ' . db_prefix() . 'master_settings WHERE title = ? AND branch_id = ? LIMIT 1',
			['branch_short_code', $branch_id]
		)->row();
		$branch_short_code = $get_branch_short_code ? $get_branch_short_code->options : '';

		// MR No is generated by generate_mr_no() after transaction commit
	...
	public function generate_mr_no($userid)
	{
		...
		for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
			...
			// If collision detected and not last attempt, wait 1 second and retry
