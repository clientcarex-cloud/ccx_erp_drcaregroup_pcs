...
class Banner_model extends App_Model {
    ...
    public function save_news_ticker($data) {

        $insert = false;
        $update = false;

        $adminIds = $data['staff_ids'] ?? '';
        $clientIds = $data['client_ids'] ?? '';

        $data['admin_area'] = !empty($adminIds) ? 1 : 0 ;
        $data['clients_area'] = !empty($clientIds) ? 1 : 0 ;
        $data['staff_ids'] = !empty($adminIds) ? serialize($adminIds) : '' ;
        $data['client_ids'] = !empty($clientIds) ? serialize($clientIds) : '' ;
        $data['news_details'] = serialize($data['news']);
        $data['start_date'] = to_sql_date($data['start_date']);
        $data['end_date'] = to_sql_date($data['end_date']);
        unset($data['news']);

        if (empty($data['id'])) {
            $insert = $this->db->insert(db_prefix() . 'news_ticker', $data);
        } else {
            $update = $this->db->where(['id' => $data['id']])->update(db_prefix() . 'news_ticker', $data);
        }
        ...
        if ($type) {
            $message = ($insert) ? _l('news_ticker_added_successfully') : _l('news_ticker_updated_successfully');
    ...
    public function get_news_ticker($id = '') {
        if (!empty($id)) {
            return $this->db->get_where(db_prefix() . 'news_ticker', ['id' => $id])->row();
        }
        return $this->db->get(db_prefix() . 'news_ticker')->result();
    }

    public function delete_news_ticker($id) {
        $this->db->delete(db_prefix() . 'news_ticker', ['id' => $id]);
        return $this->db->affected_rows() > 0 ? _l('news_ticker_deleted_successfully') : _l('something_went_wrong');
    }

    public function change_news_ticker_status($data) {
        $update = $this->db->update(db_prefix() . 'news_ticker', ['status' => 0], ['status' => 1]);
        return ($update) ? $this->db->update(db_prefix() . 'news_ticker', ['status' => $data['status']], ['id' => $data['id']]) : false;
    }
