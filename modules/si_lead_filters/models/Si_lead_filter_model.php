...
class Si_lead_filter_model extends App_Model
{
	...
	*/
	...
	{
		if($this->db->field_exists('is_default',db_prefix() . 'si_lead_filter')){
			$this->db->where('staff_id', $staff_id);
			$this->db->where('is_default', 1);
			$result = $this->db->get(db_prefix() . 'si_lead_filter')->row();
			if($result)
				return $result->id;
		}
		return "";
	...
	*/
	...
	{
		$this->db->where('staff_id',$staff_id);
		$update = $this->db->update(db_prefix() . 'si_lead_filter', ['is_default' => 0]);
		return $update;
	}
}
