...
class Patient_filters extends AdminController 
{
	public function __construct()
	{
		parent::__construct(); 
		$this->load->model('patient_filter_model');
		...
		if (!is_admin() && !has_permission('patient_filters', '', 'view')) {
			access_denied(_l('patient_filters'));
		}
	}
	...
	public function patient_filter()
	{
		...
		{
			$filter_obj = $this->patient_filter_model->get($filter_id);
			if(!empty($filter_obj))
			{
		...
		if($save_filter==1)
		{
			...
			if($filter_id!='' && is_numeric($filter_id))
				$this->patient_filter_model->update($filter_data,$filter_id);
			else					 
				$new_filter_id = $this->patient_filter_model->add($filter_data);
		}

		$list_custom_field = si_lf_get_custom_fields_from_settings();
		

		$data['title']    = _l('patient_filters');
		$data['lead_statuses'] = $this->leads_model->get_status();
		$data['lead_sources']  = $this->leads_model->get_source();
		$data['lead_countries']  = $this->patient_filter_model->get_leads_country_list();
		$data['lead_cities']  = $this->patient_filter_model->get_leads_city_list();
		$data['lead_states']  = $this->patient_filter_model->get_leads_state_list();
		$data['lead_zips']  = $this->patient_filter_model->get_leads_zip_list();
		...
		$data['filter_templates'] = $this->patient_filter_model->get_templates($current_user_id);
	...
	function table()
	{
		...
		$this->app->get_table_data(module_views_path(PATIENT_FILTERS_MODULE_NAME,'tables/patients'), $data);
	}
	...
	function list_filters()
	{
		...
		$current_user_id = get_staff_user_id();
		$data['filter_templates'] = $this->patient_filter_model->get_templates($current_user_id);
		$this->load->view('patient_list_filters', $data);
	}
	function del_lead_filter($id)
	{
		$current_user_id = get_staff_user_id();
		$this->patient_filter_model->delete($id,$current_user_id);
	...
	function settings()
	{
		...
		$this->load->view('patient_settings', $data);
