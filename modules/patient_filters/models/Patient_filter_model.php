...
class patient_filter_model extends App_Model
{
	...
	*/
	public function get($id = '')
	{
		...
		if (is_numeric($id)) {
			$this->db->where('id', $id);
			return $this->db->get(db_prefix() . 'patient_filter')->row();
		}
		return $this->db->get(db_prefix() . 'patient_filter')->result_array();
	}
	/**
	...
	*/
	public function add($data)
	{
		...
		$this->db->insert(db_prefix() . 'patient_filter', $data);
	...
	*/
	public function update($data,$filter_id)
	{
		...
		$update = $this->db->update(db_prefix() . 'patient_filter', $data);
		if ($update) {
	...
	*/
	public function delete($id,$staff_id)
	{
		$this->db->where('id', $id);
		$this->db->where('staff_id', $staff_id);
		$this->db->delete(db_prefix() . 'patient_filter');
	...
	*/
	...
	{
		if($this->db->field_exists('is_default',db_prefix() . 'patient_filter')){
			$this->db->where('staff_id', $staff_id);
			$this->db->where('is_default', 1);
			$result = $this->db->get(db_prefix() . 'patient_filter')->row();
			if($result)
				return $result->id;
		}
		return "";
	...
	*/
	...
	{
		$this->db->where('staff_id',$staff_id);
		$update = $this->db->update(db_prefix() . 'patient_filter', ['is_default' => 0]);
		return $update;
	}
}
