 ...
 * @return string
 */
function patient_format_lead_status($status, $text = false, $clean = false)
{
	if (!is_array($status)) {
		$status = patient_get_lead_status_by_id($status);
	}
 ...
 * @return array
 */
function patient_get_lead_status_by_id($id)
{
	$CI       = &get_instance();
...
function patient_lf_get_custom_field_values($fieldid,$relid='')
{
	$CI       = &get_instance();
...
function patient_lf_get_custom_fields_from_settings()
{
	$list_custom_field =[];
	if(get_option(PATIENT_FILTERS_MODULE_NAME.'_cf')!=="" )
		$list_custom_field = unserialize(get_option(PATIENT_FILTERS_MODULE_NAME.'_cf'));
