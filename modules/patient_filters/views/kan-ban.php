...
foreach ($lead_statuses as $status) {
	...
	if(!in_array('',$statuses) && !in_array($status['id'],$statuses))
		continue;		
	$total_pages = ceil($overview[$status['id']]['total_leads']/get_option('leads_kanban_limit'));
    $total_leads = $overview[$status['id']]['total_leads'];
    ...
    if (!empty($status['color'])) {
        $status_color = 'style="background:' . $status['color'] . ';border:1px solid ' . $status['color'] . '"';
            ...
            <div class="panel-heading tw-bg-neutral-700 tw-text-white"
                ...
                <?php $statusSummaryIndex = array_search($status['id'], array_column($summary, 'id'));
                if($perfex_version >= 250) echo app_format_money($summary[$statusSummaryIndex]['value'],$base_currency); ?> - <small><?php echo $summary[$statusSummaryIndex]['total'] . ' ' . _l('leads') ?></small>
            ...
            <div class="kan-ban-content-wrapper">
                <div class="kan-ban-content">
                    <ul class="status leads-status sortable" data-lead-status-id="<?php echo $status['id']; ?>">
                        <?php
                foreach ($overview[$status['id']] as $lead) {
                    $this->load->view('_kan_ban_card', ['lead' => $lead, 'status' => $status, 'base_currency' => $base_currency]);
                } ?>
                    ...
                    echo ' disabled';
                                ...
                                onclick="si_leads_kanban_load_more(<?php echo $status['id']; ?>, this, 'si_lead_filters/leads_kanban_load_more', 315, 360); return false;"
                                ;>
