...
?>
<link href="<?php echo module_dir_url('patient__filters','assets/css/patient__filters_style.css'); ?>" rel="stylesheet" />
<div id="wrapper">
	<div class="content">
		<div class="row">
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body">
						...
						<h4 class="pull-left"><?php echo _l('patient_filters'); ?> <small class="text-success"><?php echo htmlspecialchars($saved_filter_name);?></small></h4>
						<div class="btn-group pull-right mleft4 btn-with-tooltip-group" data-toggle="tooltip" data-title="<?php echo _l('si_lf_filter_templates'); ?>" data-original-title="" title="">
							...
							if(!empty($filter_templates))
							{
								...
								{
									echo "<li><a href='patient_filter?filter_id=$row[id]'>".($row['is_default']? "<i class='fa fa-star'></i> ":"")."$row[filter_name]</a></li>";
								}
							}
							else
						...
						<a href="patient_filter" class=" pull-right btn btn-info mleft4" data-toggle="tooltip" data-title="<?php echo _l('si_lf_new_info'); ?>"><?php echo _l('si_lf_new'); ?></a>
						<a href="<?php echo admin_url('patient_filters/switch_kanban/' . $switch_kanban); ?>"
                                class="btn btn-default  pull-right mleft4 hidden-xs" data-toggle="tooltip" data-placement="top"
                                data-title="<?php echo $switch_kanban == 1 ? _l('leads_switch_to_kanban') : _l('switch_to_list_view'); ?>">
                                <?php if ($switch_kanban == 1) { ?>
                                <i class="fa fa-th fa-solid fa-grip-vertical"></i>
                                <?php } else { ?>
                                <i class="fa fa-bars fa-solid fa-table-list"></i>
                                <?php }; ?>
                        </a>
						<!--lead summaray-->
						...
						<div class="col-md-2 form-group border-right" id="report-time">
								...
								<select class="selectpicker" name="report_months" id="report_months"
									...
									onchange="dt_custom_view(this.value, '.table-si-leads', 'report_months'); return false;">
							...
							<div class="col-md-2 form-group border-right" id="followup-time">
								<label for="followup_months"><?php echo _l('followup_datepicker'); ?></label><br />
								<select class="selectpicker" name="followup_months" id="followup_months"
									data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>"
									onchange="dt_custom_view(this.value, '.table-si-leads', 'followup_months'); return false;">
							...
							<div class="col-md-2 border-right">
								<label for="treatments" class="control-label"><?php echo _l('treatment'); ?></label>
								<?php 
								// Add an "Unknown" or default option like in zip example
								
								echo render_select(
									'treatments[]', // Name with [] for multiple
									$treatments,    // Options array
									array('treatment_id', 'treatment_name'), // ID and display name keys
									'',             // Label (empty because label is manually added above)
									$selected_treatments ?? array(), // Selected values
									array(
										'data-width' => '100%',
										'data-none-selected-text' => _l('leads_all'),
										'multiple' => true,
										'data-actions-box' => true,
										'onchange' => "dt_custom_view(this.value,'.table-si-leads','treatments'); return false;"
									),
									array(),        // Additional attributes
									'no-mbot',      // Additional class
									'',             // Tooltip
									false           // Include validation
								);
								?>
							</div>
							...
							<div class="col-md-2 border-right form-group <?php echo ($isKanBan ? 'hide' : '')?>">
								...
								<select name="hide_columns[]" id="hide_columns" class="selectpicker no-margin" data-width="100%" multiple>
									...
									<option value="company" <?php echo (in_array('company',$hide_columns)?'selected':'')?>><?php echo _l('name'); ?></option>
							...
							<div class="col-md-2 border-right mtop10 mbot15">
								<div class="checkbox checkbox-success" data-toggle="tooltip" title="" data-original-title="<?php echo _l('si_lf_set_as_default_info'); ?>">
									<input type="checkbox" id="si_lf_is_default" name="is_default" value="1" title="<?php echo _l('si_lf_set_as_default_info'); ?>" <?php echo ($is_default == 1?'checked':'')?>>
									<label for=""><span><?php echo _l('si_lf_set_as_default'); ?></span></label>
								</div>
							</div>
							<!--end set as default-->
							<!--start save filter-->
							<div class="col-md-4">
								<div class="checklist relative">
									<div class="checkbox checkbox-success checklist-checkbox" data-toggle="tooltip" title="" data-original-title="<?php echo _l('si_lf_save_filter_template'); ?>">
										...
										<textarea id="si_lf_filter_name" name="filter_name" rows="1" placeholder="<?php echo _l('si_lf_filter_template_name'); ?>" maxlength='100'><?php echo ($this->input->get('filter_id')?$saved_filter_name:'');?></textarea>
									</div>
								</div>
							</div>
							<!--end save filter-->
						...
						<?php if($isKanBan) { 
							echo form_hidden('sort_type', get_option('default_leads_kanban_sort')); 
							echo form_hidden('sort', (get_option('default_leads_kanban_sort') != '' ? get_option('default_leads_kanban_sort_type') : ''));
                        } ?>
						...
						<div class="panel-body">
							<div class="active kan-ban-tab" id="kan-ban-tab" style="overflow:auto;">
								<div class="kanban-leads-sort">
                                    ...
                                    <a href="#" onclick="patient_s_kanban_sort('dateadded'); return false"
                                        class="dateadded">
                                    ...
                                    |
                                    <a href="#" onclick="patient_s_kanban_sort('leadorder');return false;"
                                        class="leadorder">
                                    ...
                                    |
                                    <a href="#" onclick="patient_s_kanban_sort('lastcontact');return false;"
                                        class="lastcontact">
...
<script src="<?php echo module_dir_url('patient_filters','assets/js/patient_filters_lead_report.js'); ?>"></script>
<script>
(function($) {
"use strict";
...
<?php if(!$isKanBan){?>
initDataTable('.table-si-leads', admin_url+'patient_filters/table', undefined, undefined,
SiLeadsServerParams, <?php echo hooks()->apply_filters('patient_s_table_default_order', json_encode(array(1,'desc'))); ?>);
<?php }?>
})(jQuery);
//update lead status
function patient_s_status_update(status, lead_id) 
{
	lead_mark_as(status, lead_id);
	setTimeout(function() {
		$.get(admin_url + 'patient_filters/get_lead_status/'+lead_id, function(response) {
