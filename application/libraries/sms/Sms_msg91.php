...
class Sms_msg91 extends App_sms
{
	...
	public function send_fastAPI($numbers, $message_id, $variables_values)
	{
		$fields = array(
			"sender_id" => $this->sender_id, // Replace with your DLT sender ID
			"message" => $message_id,       // This is the DLT Template ID from Fast2SMS
			"variables_values" => $variables_values, // Example: "12345|asdaswdx"
			"route" => "dlt",
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_HTTPHEADER => array(
				"authorization: $this->auth_key", // Replace with your Fast2SMS API key
				"accept: */*",
		...
		if ($err) {
			log_message('error', 'Fast2SMS cURL Error: ' . $err);
			$this->set_error('SMS sending failed: ' . $err);
			return false;
		} else {
			$result = json_decode($response, true);
			if (!empty($result['return']) && $result['return'] == true) {
				$this->logSuccess($numbers, $variables_values);
				return $response;
			} else {
				$errorMsg = isset($result['message']) ? json_encode($result['message']) : 'Unknown error';
				$this->set_error('SMS failed: ' . $errorMsg);
				return $errorMsg;
			}
		}
