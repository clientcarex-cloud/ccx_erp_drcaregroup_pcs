...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
			...
			'(SELECT patient_status
				...
				ON ' . db_prefix() . 'clients.userid = ' . db_prefix() . 'clients_new_fields.userid 
				LIMIT 1) as patient_status',
        ...
        foreach ($rResult as $aRow) {
            ...
            if ($hasPermissionDelete) {
				$company .= '<a href="' . admin_url('client/delete/' . $aRow['userid']) . '" class="_delete" onclick="return confirm(\'Are you sure you want to delete this record?\')">' . _l('delete') . '</a>';
			}
			...
			//Unit doctor
			//$row[] = e($aRow['unit_doctor']).'_'.$aRow['userid'];
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
			...
			$row[] = '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '" data-toggle="tooltip" title="' . _l('view') . '">'
    . e($aRow['patient_status']) . '</a>';
...
function mask_mobile_number($number) {
