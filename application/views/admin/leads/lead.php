...
<div class="modal-header">
    ...
    <div class="tw-flex tw-justify-between tw-items-center">
       ...
       <?php if (isset($lead) && staff_can('view', 'leads')) { ?>
			<!--<a href="#"
                            ...
                            <div class="feed-item">
								...
								if (!empty($log['additional_data'])) {
									...
									if ($data && is_array($data)) {
										if (isset($data[1])) {
											// Remove any <a href="...">...</a> links from the message
											$formatted = preg_replace('/<a[^>]*>(.*?)<\/a>/is', '', $data[1]);
											echo "<br><span style='color:#555;'>" . nl2br(htmlspecialchars($formatted)) . "</span>";
										}
									} else {
										// fallback raw output if not serialized properly
										$formatted = preg_replace('/<a[^>]*>(.*?)<\/a>/is', '', $log['additional_data']);
										echo "<br><span style='color:#555;'>" . htmlspecialchars($formatted) . "</span>";
									}
								}
