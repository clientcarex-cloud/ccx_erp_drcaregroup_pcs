...
<ul class="dropdown-menu search-results animated fadeIn no-mtop display-block" id="top_search_dropdown">
    ...
    foreach($result as $data){
       ...
       <?php foreach($data['result'] as $_result){
        ...
        switch($data['type']){
            case 'clients':
            $output = '<a href="'.admin_url('client/get_patient_list/'.$_result['userid']).'">'.e($_result['company']) .'</a>';
            break;
            case 'contacts':
            $output = '<a href="'.admin_url('client/get_patient_list/'.$_result['userid'].'?contactid='.$_result['id']).'">'. e($_result['firstname'] .' ' . $_result['lastname']) .' <small>'.e(get_company_name($_result['userid'])).'</small></a>';
            break;
            case 'staff':
