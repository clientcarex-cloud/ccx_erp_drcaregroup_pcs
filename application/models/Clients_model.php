    ...
    public function get($id = '', $where = [])
{
    ...
    $countriesTable = db_prefix() . 'countries';
    $newFieldsTable = db_prefix() . 'clients_new_fields';
    ...
    $this->db->select("$clientsTable.*, $countriesTable.*, $contactsTable.*, $newFieldsTable.*, $customerGroupsTable.groupid, $customersGroupsTable.name as branch_name");

    $this->db->join($countriesTable, "$countriesTable.country_id = $clientsTable.country", 'left');
    $this->db->join($newFieldsTable, "$newFieldsTable.userid = $clientsTable.userid", 'left');
    $this->db->join($contactsTable, "$contactsTable.userid = $clientsTable.userid AND $contactsTable.is_primary = 1", 'left');
    $this->db->join($customerGroupsTable, "$customerGroupsTable.customer_id = $clientsTable.userid", 'left');
    $this->db->join($customersGroupsTable, "$customersGroupsTable.id = $customerGroupsTable.groupid", 'left');
    ...
    if (is_array($where) && count($where) > 0) {
        ...
        foreach ($where as $key => $val) {
            if (strpos($key, '.') === false) {
                $where_cleaned["$clientsTable.$key"] = $val;
            } else {
                $where_cleaned[$key] = $val;
            }
        }
        $this->db->where($where_cleaned);
    } elseif (is_string($where) && $where != '') {
        $this->db->where($where);
    }

    if (is_numeric($id)) {
        $this->db->where("$clientsTable.userid", $id);
        $client = $this->db->get($clientsTable)->row();
    ...
    return $this->db->get($clientsTable)->result_array();
}
    ...
    public function add($data, $withContact = false)
    {
		...
		$formatted_number = str_pad($number, 4, '0', STR_PAD_LEFT); // Pads to at least 4 digits

		$mr_no = $branch_code . '-' . $branch_short_code . '-' . $formatted_number;
		
        $contact_data = [];
        $clients_new_fields = [];
		$leadid = $data['leadid'];
		$clients_new_fields['email_id'] = $data['email'];
        ...
        if ($client_id) {
			...
			$clients_new_fields['mr_no'] = $mr_no;
			$clients_new_fields['userid'] = $client_id;
			$clients_new_fields['patient_status'] = "Active";
			$clients_new_fields['whatsapp_number'] =  $data['phonenumber'];
			
			$this->db->insert(db_prefix() . 'clients_new_fields', $clients_new_fields);
			
			$get_branch = $this->db->get_where(db_prefix() . 'lead_call_logs', array("leads_id"=>$leadid))->row();
			if($get_branch){
				$groupid = $get_branch->branch_id;
				$this->db->insert('customer_groups', [
                        'customer_id' => $client_id,
                        'groupid'     => $groupid,
                    ]);
