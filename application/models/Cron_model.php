...
class Cron_model extends App_Model
{
    ...
    private function missed_appointments()
	{
		...
		if ($branch_id) {
			$get_data = $this->db->get_where(db_prefix() . 'master_settings', [
				'title'     => 'missed_appointment',
				'branch_id' => $branch_id
			])->row();

			$missed_appointment_min = $get_data ? $get_data->options : '';

			if ($missed_appointment_min) {
				$cutoff_datetime = date('Y-m-d H:i:s', strtotime("-$missed_appointment_min minutes"));

				// Build query
				$this->db->select(db_prefix() . 'appointment.*, ' . db_prefix() . 'clients.leadid'); // Add any other client fields you need

				$this->db->from(db_prefix() . 'appointment');

				$this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid = ' . db_prefix() . 'appointment.userid', 'left');

				$this->db->where(db_prefix() . 'appointment.visit_status', 0);
				$this->db->where(db_prefix() . 'appointment.appointment_date <', $cutoff_datetime);
				$this->db->where('DATE(' . db_prefix() . 'appointment.appointment_date)', date('Y-m-d')); // Only today
				$this->db->where(db_prefix() . 'appointment.branch_id', $branch_id);
				$this->db->where(db_prefix() . 'appointment.sent_missed_notification', 0);


				$appointments = $this->db->get()->result();
				
				$status_name = "Missed Appointment";
				...
				foreach($appointments as $appointment){
					...
					if($status_id){
						...
						$this->client_model->patient_journey_log_event($userid, $status_name, $status_id, $communication_data);
						
						$update = array(
						"sent_missed_notification" => 1
						);
						$this->db->where(array("appointment_id"=>$appointment_id));
						$this->db->update(db_prefix() . 'appointment', $update);
					}
