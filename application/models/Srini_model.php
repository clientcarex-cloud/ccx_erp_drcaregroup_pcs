...
class Srini_model extends App_Model
{
    ...
    /* Srini Code for prescription*/
    public function get_prescription()
    {
        //$this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'prescription.addedfrom');
        //$this->db->where('rel_id', $rel_id);
        //$this->db->where('rel_type', $rel_type);
        //$this->db->order_by('dateadded', 'desc');

        $prescription = $this->db->get(db_prefix() . 'prescription')->result_array();

        return hooks()->apply_filters('get_prescription', $prescription, ['rel_id' => $rel_id, 'rel_type' => $rel_type]);
    }

    public function add_prescription($data, $rel_type, $rel_id)
    {
      /* // $data['dateadded']   = date('Y-m-d H:i:s');
        //$data['addedfrom']   = get_staff_user_id();
       // $data['rel_type']    = $rel_type;
       ...
       // $data = hooks()->apply_filters('create_prescription_data', $data, $rel_type, $rel_id);

        $this->db->insert(db_prefix() . 'prescription', $data);
        $insert_id = $this->db->insert_id();

        if ($insert_id) {
           // hooks()->do_action('prescription_created', $insert_id, $data);

            return $insert_id;
        }

        return false;*/
        //$test = $this->db->get(db_prefix() . 'prescription')->result_array();
       // print_r($this->input->post());
        ...
        $this->db->insert(db_prefix() . 'prescription', $data);
        $insert_id = $this->db->insert_id();
        //retrun $this->db->insert_id();
    }

    public function edit_prescription($data, $id)
    {
        hooks()->do_action('before_update_note', [
            'data' => $data,
            'id'   => $id,
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'prescription', $data = [
            'prescription' => nl2br($data['prescription']),
        ]);

        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('prescription_updated', $id, $data);
    ...
    {
        hooks()->do_action('before_delete_prescription', $prescription_id);

        $this->db->where('id', $prescription_id);
        $note = $this->db->get(db_prefix() . 'prescription')->row();
        ...
        $this->db->where('id', $prescription_id);
        $this->db->delete(db_prefix() . 'prescription');
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('prescription_deleted', $prescription_id, $prescription);
