...
class Misc_model extends App_Model
{
    ...
    public function perform_search($q)
    {
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
			...
			// Clients
			$this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'clients')) . ',' . get_sql_select_client_company() . ', ' . db_prefix() . 'clients_new_fields.mr_no, ' . db_prefix() . 'clients_new_fields.alt_number1');
			...
			// Add the new table to the join
			$this->db->join(db_prefix() . 'clients_new_fields', db_prefix() . 'clients_new_fields.userid = ' . db_prefix() . 'clients.userid', 'left');
			...
			$this->db->where('(company LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
				...
				OR ' . db_prefix() . 'clients_new_fields.mr_no LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
        ...
        if (is_staff_member()) {
            ...
            if (!startsWith($q, '#')) {
                $this->db->where('(name LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                    ...
                    OR lead_alternate_number LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\'
                    )');
            } else {
                $this->db->where('id IN
                    (SELECT rel_id FROM ' . db_prefix() . 'taggables WHERE tag_id IN
