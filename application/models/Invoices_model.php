...
class Invoices_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        $this->db->select('
				*,
				' . db_prefix() . 'currencies.id AS currencyid,
				' . db_prefix() . 'invoices.id AS id,
				' . db_prefix() . 'currencies.name AS currency_name,
				branch.name AS payment_branch_name
			', false); // 👈 false = do not auto-escape

        $this->db->from(db_prefix() . 'invoices');
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoices.currency', 'left');
		$this->db->join(db_prefix() . 'customers_groups as branch', 'branch.id = invoices.branch_id', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                ...
                $client          = $this->client_model->get($invoice->clientid);
