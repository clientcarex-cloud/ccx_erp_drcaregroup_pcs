...
class Leads extends AdminController
{
    ...
    public function index($id = '')
    {
        ...
        $data['pincodes']  = $this->leads_model->get_pincodes();
        ...
        if ($this->input->post()) {
            if ($id == '') {
				if (staff_cant('create', 'leads')) {
					access_denied('Add Lead');
				}
				/* $phonenumber = $this->input->post('phonenumber');
				$check_lead = $this->db->get_where(db_prefix() . 'leads', array("phonenumber"=>$phonenumber))->row();
				if($check_lead){
					 $message = $id ? _l('lead alredy exists', _l('lead')) : '';
				...
				if (staff_cant('edit', 'leads')) {
					access_denied('Edit Lead');
				}
    ...
    private function _get_lead_data($id = '')
    {
        ...
        $data['base_currency'] = get_base_currency();
		//New Code
		$data['branches'] = $this->leads_model->get_branches();
		$data['treatments'] = $this->leads_model->get_treatments();
		$data['patient_response'] = $this->leads_model->get_patient_response();
		$data['doctors'] = $this->leads_model->get_doctors();
		$data['appointment_type'] = $this->leads_model->get_appointment_type();
		
		$data['staff_list']   = $this->leads_model->get_staff_list();
		$data['patient_list'] = $this->leads_model->get_patient_list();
        ...
        $data['consultation_fees']  = $this->leads_model->get_consultation_fee();
		$data['patient_response'] = $this->leads_model->get_patient_response();
        ...
        if (is_numeric($id)) {
            ...
            $data['lead']          = $lead;
			$data['lead_with_doctor'] = $this->leads_model->get_lead_with_doctor($id);
	...
	public function get_cities_by_state()
	{
		...
		if ($state_id) {
			...
			$cities = $this->db->get(db_prefix() . 'city')->result_array();
			echo json_encode($cities);
		}
	}
	
	public function get_country_calling_code()
	{
		...
		if ($country) {
			echo json_encode(['calling_code' => $country->calling_code]);
		} else {
			echo json_encode(['calling_code' => '']);
		}
	}
	public function search_phone_matches()
	{
		if ($this->input->is_ajax_request()) {
			$phone = $this->input->post('phone');
			$this->db->like('phonenumber', $phone);
			$this->db->limit(10);
			$matches = $this->db->get(db_prefix() . 'leads')->result_array();
